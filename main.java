package Basic_Structure_and_Syntax;

public class main { 
     public static void main(String[] args) {
        System.out.println("Hello, World!");
        System.out.println("Welcome to Java programming!");
        System.out.println("Im Phosphophyllite, nice to meet you!");
        System.out.println("Im 400 years old, but I look like a 14 year old!");
        System.out.println("See you after 10,000 years!");
        System.out.println("I have the power to send you to a place filled with nothing but anguish and suffering,");
        System.out.println("But I refuse to do that. Farewellll.");
             

        
    }    


    
}
